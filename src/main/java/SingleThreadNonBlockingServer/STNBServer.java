...
public class STNBServer {
    ...
    public static void main(String[] args){
        // Thread that will display the number of online users every 5 seconds
        new Thread(() -> {
            while(true){
                try {
                    Thread.sleep(5000);
                    System.out.println("# " + onlineUsersNbr + " users online");
...
//            int validOps = serverSocketChannel.validOps();
    ...
    private static void handleAccept(SelectionKey selectionKey, Selector selector) throws IOException {
        ...
        byteBuffer.put((NOTICE_USER_IDENTIFIER + user.getId()).getBytes());
        byteBuffer.flip();
